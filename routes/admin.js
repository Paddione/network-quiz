...
// Update option
router.put('/api/options/:id', isAdmin, async (req, res) => {
    const optionId = req.params.id;
    const { option_text, is_correct, sequence_number } = req.body;

    try {
        // Get the question_id for this option
        const optionResult = await db.query('SELECT question_id FROM options WHERE id = $1', [optionId]);

        if (optionResult.rows.length === 0) {
            return res.status(404).json({ error: 'Option not found' });
        }

        const question_id = optionResult.rows[0].question_id;

        // If this option is marked correct, update all other options to be incorrect
        if (is_correct) {
            await db.query(
                'UPDATE options SET is_correct = FALSE WHERE question_id = $1 AND id != $2',
        ...
        // Update option
        const result = await db.query(
            'UPDATE options SET option_text = $1, is_correct = $2, sequence_number = $3 WHERE id = $4 RETURNING *',
            [option_text, is_correct, sequence_number, optionId]
        );
...
// Delete option
router.delete('/api/options/:id', isAdmin, async (req, res) => {
    const optionId = req.params.id;
